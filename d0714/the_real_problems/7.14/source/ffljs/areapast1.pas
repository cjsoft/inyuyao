...
function efsj(f1,f2:longint):int64;
var l,r,m:longint;ans:int64;
begin
	...
	ans:=weifen(f1,f2,m);
	if ans<0 then r:=m
	else if ans>0 then l:=m
	else if ans=0 then exit(sarea(a[tb[f1]],a[tb[m]],a[tb[f2]]));
	m:=(l+r)div 2;
    end;
    exit(sarea(a[tb[f1]],a[tb[r]],a[tb[f2]]));
end;
function sifang(f1,f2:longint):int64;
begin
    exit(efsj(f1,f2)+efsj(f2,f1+tb[0]));
end;
function effx(var f1,f2:longint):int64;
var an1,an2,an3:int64;
begin
