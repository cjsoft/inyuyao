...
int main()
{
	...
	S=n*m*3+1;S1=S+1;T=S+2;
	adde(S,S1,2,0);
	for (int i=1;i<=m;i++) 
		adde(S1,id(1,i,0),1,0),adde(id(n,i,1),T,1,0);
	for (int i=1;i<=n;i++)
		for (int j=1;j<=m;j++)
	...
	for (int i=1;i<n;i++)
	{
		for (int j=1;j<=m;j++) 
		{
			adde(id(i,j,1),id(i,j,2),1,0);
			adde(id(i,j,2),id(i+1,j,0),1,0);
		}
		for (int j=1;j<m;j++)
		{
			adde(id(i,j,2),id(i,j+1,2),2,K);
			adde(id(i,j+1,2),id(i,j,2),2,K);
		}
	}
	while (spfa())
		mcf();
