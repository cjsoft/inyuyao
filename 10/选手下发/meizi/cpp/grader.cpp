...
int ASK(int x,int y)
{
	js++;if (js>10000000) {printf("too many queries\n");exit(0);}
...
void score(int s)
{	
	if (s<=6050) printf("ok,100\n");
	else if (s<=16050) printf("ok,70\n");
	else if (s<=48050) printf("ok,50\n");
	else if (s<=180050) printf("ok,30\n");
	else if (s<=4000050) printf("ok,10\n");
	else printf("ok,0,too many queries\n");
}
int main()
{
	for (int t=1;t<=20;t++)
	{
		js=0;
		freopen(change("sampletest"+chg[t]+".in"),"r",stdin);
		for (int i=1;i<=2000;i++)
		for (int j=1;j<=2000;j++)
		read(a[i][j]);
		fclose(stdin);
		pair<int,int> p=FINDMXPOINT();
		if (check(p)) score(js);
