...
struct treaps
{
	int son[90001000][2],size[90001000],num[90001000],rd[90001000],tt;
	int newnode(int s=0)
	{
		if (tt>90000000) throw(0); 
		int t=++tt;
		son[t][0]=son[s][0];
		son[t][1]=son[s][1];
		size[t]=size[s];
...
struct segs
{
	...
	int work(int root,int s,int k,int pl=1,int pr=n)
	{
		if (pl==pr)
		{
			if (treap.getsize(ins[root])<k) {printf("Error\n");return -INF;}
			return treap.findkth(ins[root],treap.getsize(ins[root])-k+1);
		}
		pushdown(root);
		int mid=(pl+pr)>>1;
		if (s<=mid) return work(l[root],s,k,pl,mid);
		else return work(r[root],s,k,mid+1,pr);
	}
